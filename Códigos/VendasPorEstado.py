...
df = pd.read_excel('Vendas_por_Estado.xlsx', engine='openpyxl')
...
# Gerar um gráfico de barras para a quantidade de veículos vendidos por estado
plt.figure(figsize=(12, 8))
plt.bar(df['Estado'], df['Quantidade'])
plt.xlabel('Estado')
plt.ylabel('Quantidade')
plt.title('Quantidade de Veículos Vendidos por Estado')
plt.xticks(rotation=90)
plt.tight_layout()

# Salvar o gráfico como um arquivo de imagem
plt.savefig('vendas_por_estado.png')
